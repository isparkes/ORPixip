...
insert into RATE_PRICE_PIVOT (PRICE_MODEL,STEP,TIER_FROM,TIER_TO,BEAT,FACTOR,CHARGE_BASE)
(
  select distinct PRICE_GROUP,1,0,999999999,1,RATE_PRICE,1024 from PRICE_MAP where RATING_TYPE = 'MBBeat1kB'
);
...
(
  select distinct PRICE_GROUP,PRICE_GROUP,'DUR','MONEY',888,'TIERED',0,1 from PRICE_MAP where RATING_TYPE in ('Setup,Beat60,Beat1','Setup,Beat1','Beat1','Beat30,Beat1','Beat60,Beat15','Beat60,Beat30','Setup,Beat60')
);
...
INSERT INTO RUM_MAP (PRICE_GROUP,PRICE_MODEL,RUM,RESOURCE,RESOURCE_ID,RUM_TYPE,CONSUME_FLAG,STEP)
(
  select distinct PRICE_GROUP,PRICE_GROUP,'EVT','SEK',888,'EVENT',0,1 from PRICE_MAP where RATING_TYPE in ('Event')
);

INSERT INTO RUM_MAP (PRICE_GROUP,PRICE_MODEL,RUM,RESOURCE,RESOURCE_ID,RUM_TYPE,CONSUME_FLAG,STEP)
(
  select distinct PRICE_GROUP,PRICE_GROUP,'SEK','SEK',888,'TIERED',0,1 from PRICE_MAP where RATING_TYPE in ('Markup','Setup,Markup')
);
